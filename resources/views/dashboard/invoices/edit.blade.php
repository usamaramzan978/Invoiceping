...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">Edit Invoice</h1>
                            ...
                            <div class="row gy-3">
                                                ...
                                                <div class="col-xl-12 choices-control">
                                                    ...
                                                    <select class="form-control select2-invoice-currency" data-trigger
                                                        ...
                                                        <option value="PKR" data-symbol="₨"
                                                            {{ old('currency', $invoice->currency) == 'PKR' ? 'selected' : '' }}>
                                                            PKR - Pakistani Rupee
                                                        </option>
                                                        <option value="GBP" data-symbol="£"
                                                            {{ old('currency', $invoice->currency) == 'GBP' ? 'selected' : '' }}>
                                                            GBP - British Pound
                                                        </option>
                                                        <option value="INR" data-symbol="₹"
                                                            {{ old('currency', $invoice->currency) == 'INR' ? 'selected' : '' }}>
                                                            INR - Indian Rupee</option>
                                                        <option value="AED" data-symbol="د.إ"
                                                            {{ old('currency', $invoice->currency) == 'AED' ? 'selected' : '' }}>
                                ...
                                <div class="col-xl-3">
                                    <label for="invoice-date-issued" class="form-label">Date Issued <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control form-control-light"
                                        id="invoice-date-issued" name="issue_date" placeholder="Choose date"
                                ...
                                <div class="col-xl-3">
                                    ...
                                    <input type="text" class="form-control form-control-light" id="invoice-date-due"
                                        name="due_date" placeholder="Choose date"
                                    ...
                                    <div class="col-xl-12">
                                        ...
                                        <div class="input-group mb-2">
                                            ...
                                            <select class="form-select form-select-sm" id="discount-type">
                                                <option value="fixed"
                                                    {{ old('discount_type', $invoice->discount_type?->value) == 'fixed' ? 'selected' : '' }}>
                                                    Fixed ($)</option>
                                                <option value="percent"
                                                    {{ old('discount_type', $invoice->discount_type?->value) == 'percent' ? 'selected' : '' }}>
                                                    Percent (%)</option>
                                            </select>
                                        </div>
                                        <div class="d-flex justify-content-between mb-2">
                                ...
                                <div class="col-xl-12">
                                    <div>
                                        ...
                                        <textarea class="form-control form-control-light" name="notes" id="invoice-note" rows="3">{{ old('notes', $invoice->notes) }}</textarea>
                                    </div>
                                </div>

                                <!-- Hidden inputs for calculated values -->
                                <input type="hidden" name="discount_type" id="discount-type-hidden"
                                    value="{{ old('discount_type', $invoice->discount_type?->value ?? 'fixed') }}">
...
@section('scripts')
    ...
    @php
        ...
        $invoiceDataArray = [
            ...
            'discount_type' => $invoice->discount_type?->value ?? 'fixed',
    ...
    <script>
        const InvoiceManager = {
            currencySymbol: '$',
            itemCount: 0,
            invoiceData: @json($invoiceDataArray),
            ...
            setupClient() {
                ...
                // Initialize client email and phone on page load
                const option = $('#client_id').find(':selected');
                if (option.length > 0) {
                    $('#client-mail').val(option.data('email') || '');
                    $('#client-phone').val(option.data('phone') || '');
            ...
            setupCurrency() {
                ...
                // Update currency symbol from invoice data
                const currencyMap = {
                    'USD': '$',
                    'EUR': '€',
                    'PKR': '₨',
                ...
                if (this.invoiceData.currency && currencyMap[this.invoiceData.currency]) {
                    this.currencySymbol = currencyMap[this.invoiceData.currency];
                }
            },
                    ...
                    <tr class="invoice-item" data-item-id="${itemId}">
                        <td>
                            <input type="text" class="form-control form-control-light product-name"
                                name="items[${itemId}][name]" placeholder="Enter Product Name"
                                value="${this.escapeHtml(itemData.name)}" required>
                        </td>
                        <td>
                            <textarea rows="1" class="form-control form-control-light product-description"
                                name="items[${itemId}][description]" placeholder="Enter Description">${this.escapeHtml(itemData.description || '')}</textarea>
                        </td>
                        ...
                        <td>
                            <input type="text" class="form-control form-control-light product-price"
                                name="items[${itemId}][unit_price]" placeholder="0.00" value="${itemData.unit_price}" required>
                        </td>
                        <td>
                            <input type="text" class="form-control form-control-light product-total"
                                placeholder="0.00" value="${(itemData.quantity * itemData.unit_price).toFixed(2)}" readonly>
                        </td>
                        <td>
            ...
            initializeValues() {
                ...
                // Wait a bit for DOM to be ready, then set discount and tax
                setTimeout(() => {
                    const subtotal = parseFloat($('#subtotal-value').val()) || 0;

                    // Set discount
                    if (this.invoiceData.discount_amount > 0 && subtotal > 0) {
                        let discountValue = 0;
                        if (this.invoiceData.discount_type === 'percent') {
                            // Reverse calculate: discount_value = (discount_amount / subtotal) * 100
                            discountValue = (this.invoiceData.discount_amount / subtotal) * 100;
                        } else {
                            // Fixed discount
                            discountValue = this.invoiceData.discount_amount;
                        }
                        $('#discount-input').val(discountValue.toFixed(2));
                        $('#discount-type').val(this.invoiceData.discount_type);
                        $('#discount-type-hidden').val(this.invoiceData.discount_type);
                    }

                    // Set tax
                    if (this.invoiceData.tax_amount > 0) {
                        const taxableAmount = subtotal - this.invoiceData.discount_amount;
                        if (taxableAmount > 0) {
                            // Reverse calculate: tax_percentage = (tax_amount / taxable_amount) * 100
                            const taxPercentage = (this.invoiceData.tax_amount / taxableAmount) * 100;
                            $('#tax-input').val(taxPercentage.toFixed(2));
                            $('#tax-percentage-hidden').val(taxPercentage.toFixed(2));
                        }
                    }

                    // Recalculate totals with discount and tax
                    this.calculateTotals();
                }, 100);
            }
        };
